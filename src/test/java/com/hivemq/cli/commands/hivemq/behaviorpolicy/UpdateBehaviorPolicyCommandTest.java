...
public class UpdateBehaviorPolicyCommandTest {
    ...
    private final @NotNull CommandLine commandLine =
            new CommandLine(new UpdateBehaviorPolicyCommand(hiveMQRestService, outputFormatter, gson));
    ...
    void call_taskSuccessful_return0() throws ApiException {
        ...
        verify(behaviorPoliciesApi).updateBehaviorPolicy(eq("policy-1"), any());
    }

    @Test
    void call_taskFailed_return1() throws ApiException {
        when(behaviorPoliciesApi.updateBehaviorPolicy(eq("policy-1"), any())).thenThrow(ApiException.class);
