...
public class GetBehaviorStateCommandTest {
    ...
    private final @NotNull CommandLine commandLine =
            new CommandLine(new GetBehaviorStateCommand(hiveMQRestService, outputFormatter));

    @BeforeEach
    void setUp() {
        TestLoggerUtils.resetLogger();
        when(hiveMQRestService.getBehaviorStateApi(any(), anyDouble())).thenReturn(stateApi);
    }
    ...
    void call_idEmpty_error() {
        ...
        verify(outputFormatter).printError(eq("The client id must not be empty."));
    }

    @Test
    void call_urlAndRateLimitPassed_usedInApi() {
        assertEquals(0, commandLine.execute("--rate=123", "--url=test-url", "--id=client-1"));
        verify(hiveMQRestService).getBehaviorStateApi(eq("test-url"), eq(123d));
    }

    @Test
    void call_taskSuccessful_return0() throws ApiException {
        assertEquals(0, commandLine.execute("--id=client-1"));
        verify(stateApi).getClientState(eq("client-1"));
    }

    @Test
    void call_taskFailed_return1() throws ApiException {
        when(stateApi.getClientState(any())).thenThrow(ApiException.class);
        assertEquals(1, commandLine.execute("--id=client-1"));
    }
}
