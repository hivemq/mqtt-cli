...
public class GetBehaviorPolicyCommandTest {
    ...
    private final @NotNull CommandLine commandLine =
            new CommandLine(new GetBehaviorPolicyCommand(hiveMQRestService, outputFormatter));
    ...
    void call_multipleFields_success() throws ApiException {
        ...
        verify(behaviorPoliciesApi).getBehaviorPolicy(eq("policy-1"), eq("id,createdAt"));
    }

    @Test
    void call_taskSuccessful_return0() throws ApiException {
        assertEquals(0, commandLine.execute("--id=policy-1"));
        verify(behaviorPoliciesApi).getBehaviorPolicy(eq("policy-1"), isNull());
    }
