...
public class PublishCommand extends AbstractConnectFlags implements MqttAction, Publish {
    ...
    @CommandLine.Option(names = {"-m", "--message"}, converter = ByteBufferConverter.class, required = true,
            description = "The message to publish", order = 1)
    private @NotNull ByteBuffer message;
    ...
    public @NotNull String toString() {
        return getClass().getSimpleName() + "{" + connectOptions() + ", topics=" + Arrays.toString(topics) + ", qos=" +
                Arrays.toString(qos) + ", message=" + new String(message.array(), StandardCharsets.UTF_8) +
