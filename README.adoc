...
```
hmq> con -i myClient -h broker.hivemq.com
myClient@localhost> exit  # client is still connected
hmq> dis -i myClient -h broker.hivemq.com
```
...
| ``-s``   | ``--stay``| The subscribe will block the console and wait for publish messages to print.

Press enter to exit the subscription context. Note that this will also unsubscribe the previously subscribed topics. | ``false``
