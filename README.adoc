...
NOTE: In Shell-Mode use the <<ContextPublish>> equivalent.
...
NOTE: In Shell-Mode use the <<ContextSubscribe>> equivalent.
...
```
{ hmq | clientID }> switch  -i <identifier>
                            [-h <host>]
                            [-p <port>]
                            [-V <version>]
```
...
client2@broker.hivemq.com> switch -i client1
client1@localhost>
```
...
NOTE: In Shell-Mode the **-d** and **-v** options for every command will be overridden the default shell debug level.
