...
|        | `--message-type`  | Only used with '--type PROTOBUF'. The Protobuf message type to use for the schema.                                                           |         |            When `--type` = `PROTOBUF`, the `--message-type string’ must match the string that defines the custom data structure in the originating .proto file. For example, `--message-type GpsCoordinates’.           |
